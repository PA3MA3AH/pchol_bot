...
async def try_use_check(check_id: int, user_id: int):
    """Атомарно помечает чек использованным. Возвращает dict с данными чека если успешно, None если уже использован или не найден."""
    async with db.pool.acquire() as conn:
        async with conn.transaction():
            row = await conn.fetchrow(
                "SELECT * FROM checks WHERE id=$1 AND is_used=FALSE FOR UPDATE", check_id
            )
            if not row:
                return None
            await conn.execute(
...
async def use_check(check_id: int, user_id: int):
    # legacy, для совместимости — просто вызывает try_use_check без использования результата
    await try_use_check(check_id, user_id)
