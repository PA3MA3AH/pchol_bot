...
async def deduct_bees(user_id: int, n: int) -> bool:
    """Атомарно списывает пчолы. Возвращает True если хватило баланса."""
    await ensure_user(user_id)
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            "UPDATE users SET bees = bees - $1 WHERE user_id=$2 AND bees >= $1 RETURNING bees",
            n, user_id,
        )
        return row is not None


async def deduct_bees_and_add(user_id: int, n: int, recipient_id: int) -> bool:
    """Атомарно списывает у sender и начисляет recipient в одной транзакции."""
    await ensure_user(user_id)
    ...
    async with db.pool.acquire() as conn:
        async with conn.transaction():
            ...
            if row is None:
                return False
            await conn.execute(
                "UPDATE users SET bees = bees + $1 WHERE user_id=$2", n, recipient_id
            )
        return True
...
async def deduct_honey(user_id: int, amount: float) -> bool:
    """Атомарно списывает мёд. Возвращает True если хватило."""
    await ensure_user(user_id)
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            "UPDATE users SET honey = honey - $1 WHERE user_id=$2 AND honey >= $1 RETURNING honey",
            amount, user_id,
        )
        return row is not None


async def deduct_honey_and_add(user_id: int, amount: float, recipient_id: int) -> bool:
    """Атомарно списывает мёд у sender и начисляет recipient."""
    await ensure_user(user_id)
    await ensure_user(recipient_id)
    async with db.pool.acquire() as conn:
        async with conn.transaction():
            ...
            if row is None:
                return False
            await conn.execute(
                "UPDATE users SET honey = honey + $1 WHERE user_id=$2", amount, recipient_id
            )
        return True
